...
const ConfirmModal: React.FC<ConfirmModalProps> = ({ isOpen, title, message, onConfirm, onCancel, confirmText = "执行操作" }) => {
      ...
      <div className="w-full max-w-sm bg-[#21222c] border border-[#ff5545]/30 rounded-2xl shadow-[0_20px_50px_rgba(255,85,69,0.2)] overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-8 space-y-6">
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <div>
              <h3 className="text-lg font-black text-[#f8f8f2] uppercase tracking-tighter italic">{title}</h3>
              <p className="text-xs text-[#6272a4] font-mono mt-2 leading-relaxed uppercase">{message}</p>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-4 border-t border-[#44475a]">
            <button 
              onClick={onCancel} 
              className="flex-1 py-3 text-[10px] font-black text-[#6272a4] hover:text-[#f8f8f2] uppercase tracking-widest transition-colors"
            >
              放弃请求
            </button>
            <button 
