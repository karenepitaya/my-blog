...
export const ProfileTab: React.FC = () => {
  ...
  const handleUrlParse = () => {
    ...
    img.onerror = () => {
        setIsLoadingUrl(false);
        alert("无法加载图片资源。请确认链接有效，且目标服务器允许跨域 (CORS) 访问。");
    };
  ...
  const generateCrop = () => {
      ...
      try {
        ...
        setShowCropModal(false);
          ...
          alert("裁剪失败：图片资源跨域受限 (Tainted Canvas)。建议下载图片后使用本地上传。");
      }
  };
    ...
    <div className="space-y-8 animate-fade-in relative">
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] bg-danger/10 text-danger px-2 py-1 rounded border border-danger/20 font-bold">ROOT</span>
        </div>
      ...
      {hoveredSkill && (
          ...
          >
              ...
              <span className="text-[10px] text-slate-500 font-mono pl-4">{hoveredSkill.def.category}</span>
          </div>
      )}
